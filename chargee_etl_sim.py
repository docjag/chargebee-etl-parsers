...
@Author: Sohel Mahmud
@Date: June 7 2019
@Description: Chargebee ETL Simulation
...
def create_table_invoice(conn):
    ...
    # fields.append('base_currency_code')
    # fields.append('new_sales_amount')
    # fields.append('is_digital')
...
def insert_new_field(conn, table_name, col_name, col_type):
    ...
    try:
        c = conn.cursor()
        c.execute(sql)
        print('A new column %s is added in %s tables uccessfully' %(col_name, table_name))
